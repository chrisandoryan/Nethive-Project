...
_SLOW_TIMESTAMP_CRE = re.compile(r"#\s+Time:\s+(" + _LOCAL_DATE_PAT + r")")
...
class SlowQueryLog(LogParserBase):
    ...
    def _parse_timestamp(self, line, entry):
        ...
        try:
            # If time-format: # Time: 190711 13:28:00
            entry['datetime'] = datetime.datetime.strptime(info[0], "%y%m%d %H:%M:%S")
        except ValueError:
            # If time-format: # Time: 2019-07-12T05:01:46.91 ISO8601
            entry['datetime'] = dateutil.parser.parse(info[0]) # datetime.datetime.strptime(info[0], "%Y-%m-%dT%H:%M:%S.%z")
